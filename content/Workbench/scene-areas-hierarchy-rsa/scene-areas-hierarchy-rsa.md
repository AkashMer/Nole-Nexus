...
authors:
  - Dr. Akash Mer
source: https://github.com/AkashMer/workbench/tree/main/scene-areas-hierarchy-rsa
---
...
These areas span the low-level to high-level feature hierarchy for scene perception([Epstein & Baker, 2019](#ref-Epstein_2019)).
...
The raw data([Zhang & Naya, 2022](#ref-Zhang_2022)) is structured as follows:
...
There are `19` participants in total. The ideal cohort size for a representational map estimation is 1 if sufficient number of neurons/voxels are recorded([Noda et al., 2024](#ref-Noda_2024-03-22)). The following algorithm is to be used to rank each participant:

- Exclude if mean **Frame Displacement** (FD) \> 0.2;
- Rank remaining using a rank-sum approach based on the **maximum standard deviation of the frame-to-frame DVARS profile([Power et al., 2012](#ref-Power_2012)) across runs** (penalizes sudden spikes) and **HND trial performance** (secondary task engagement)
...
Participant `26`’s raw DICOM data was extracted, converted to NIfTI (using `dcm2niix`([Li et al., 2016](#ref-Li_2016))), and organized into BIDS format([Gorgolewski et al., 2016](#ref-Gorgolewski_2016)). The resulting BIDS dataset was then preprocessed using `fMRIPrep`([Esteban et al., 2019](#ref-Esteban_2019)) with FreeSurfer surface reconstruction([Dale et al., 1999](#ref-Dale_1999); [Fischl et al., 2002](#ref-Fischl_2002); [Fischl & Dale, 2000](#ref-Fischl_2000); [Ségonne et al., 2004](#ref-Segonne_2004)). Same data quality metrics which were computed for the raw data are computed again utilizing fMRIPrep preprocessed output:
...
The visfAtlas([<span class="nocase">Rosenke et al.</span>, 2021](#ref-Rosenke_2021)) is a probabilistic atlas of human occipito-temporal visual cortex. I will be using the FreeSurfer label files for bilateral primary and secondary visual areas: **V1v**, **V1d**, **V2v**, **V2d**.

The [Place memory parcels and localizer](https://osf.io/xmhn7/)([Steel, 2024](#ref-Steel_2024)) provides probabilistic parcels for bilateral **OPA**, **PPA**, and **MPA/RSC**.
...
```
    ...
    Warning: NIftI time units are 'unknown'. Assuming seconds for time dimension.
...
The resulting per-run $\beta$ patterns are converted into a representational dissimilarity matrix (RDM) using crossnobis distance from **Python Representational Similarity Analysis toolbox** `rsatoolbox`([Bosch et al., 2025](#ref-Bosch_2025)). GLM residuals were supplied for noise normalization. A crosswise single-trial correlation approach which allows the diagonal of the matrix to serve as a trial-to-trial reliability measure([Noda et al., 2024](#ref-Noda_2024-03-22)) was not implemented due to the inherent low SNR of fMRI BOLD signal. The crossnobis approach serves a similar purpose by using cross-validation folds across runs ie. the estimates are no longer biased by noise within each run.
...
![](/Workbench/scene-areas-hierarchy-rsa/scene-areas-hierarchy-rsa_files/figure-commonmark/fig-rdm-heatmaps-output-1.png)

Figure 1: **ROI-wise RDM** for participant 26: *Walking direction forms the large blocks, map forms the small blocks within. Brackets show number of voxels in each area*
...
The overall textured structure is present as expected for primary and secondary areas, but the flat heatmaps of the scene-selective areas are contradictory to their function in scene processing([Epstein & Baker, 2019](#ref-Epstein_2019)). The low values of crossnobis distance are expected given the categorical nature of the trial conditions and one subject analysis. A robustness check using permutations with shuffled data across conditions is needed to confirm absence of any noise contribution.
...
![](/Workbench/scene-areas-hierarchy-rsa/scene-areas-hierarchy-rsa_files/figure-commonmark/fig-rdm-permutation-output-1.png)

Figure 2: **RDM Robustness Check**: *mean off-diagonal crossnobis distance (dot) against a condition shuffled null distribution (violin) per ROI. Green color signifies that the ROI survived the robustness check.*
...
The participation ratio (PR)([Gao et al., 2017](#ref-Gao_2017)) estimates the effective number of dimensions a ROI’s population response uses to separate the conditions. This requires a similarity matrix which is computed by expressing the crossnobis distances relative to the average position across all conditions ie. the center of all points in the representational space, which is 12-dimensional in this case. The computed PR is normalized using 11 dimensions since the above centering removes one dimension.
...
![](/Workbench/scene-areas-hierarchy-rsa/scene-areas-hierarchy-rsa_files/figure-commonmark/fig-pathway-mds-output-1.png)

Figure 3: **Areawise-MDS representational maps along the pathway**: *Top row colored by walking direction, bottom row colored by map (position of the characters). Each column is Procrustes-aligned to the previous ROI’s MDS for comparison.*
...
- **Low-Level Model**: Uses left V1d and V2d as predictors; assumes a purely feed-forward network([Serre et al., 2007](#ref-Serre_2007))
...
![](/Workbench/scene-areas-hierarchy-rsa/scene-areas-hierarchy-rsa_files/figure-commonmark/fig-mr-rsa-output-1.png)

Figure 4: **MR-RSA model comparison**: *Mean Cosine Similarity with left OPA RDM, low-level vs hierarchical model*

</div>

<a href="#fig-mr-rsa" class="quarto-xref">Figure 4</a> shows that hierarchical model is more similar to left OPA RDM than the low-level model, but the 95% confidence intervals overlap. The following interpretations can be made:

- Left OPA’s representational structure is built from features which are dependent on the low-level features used by V1 and V2. Thus the concerns put forth by ([Epstein & Baker, 2019](#ref-Epstein_2019)) regarding the overlapping nature of sensitives of scene selective areas to both low-level features and higher-order processes should be explored in more detail.
- The coarse categorical stimuli information provided with the dataset may be insufficient to answer questions about scene areas. The bare nature of the scene itself may be contributing to the flat nature of the scene area RDMs. The risk of such highly constrained stimuli thus go beyond being generalizable to natural scene stimuli([Epstein & Baker, 2019](#ref-Epstein_2019)).
- The number of voxels may not be sufficient to allow single subject analysis in this dataset.

Example stimulus videos are provided as supplementary material by both ([Zhang & Naya, 2020](#ref-Zhang_2020)) and ([Zhang et al., 2022](#ref-Zhang_2022a)) but do not cover all 12 map-direction combinations. I hereby suggest a plan to work around this limitation by replacing the characters with uniform color pixels while preserving the character silhouettes. The modified video can then serve as a good proxy for the original stimulus structure allowing extracttion of exact low-level features shown to the participant.
...
<div id="fig-rdm-heatmaps-pooled">

![](/Workbench/scene-areas-hierarchy-rsa/scene-areas-hierarchy-rsa_files/figure-commonmark/fig-rdm-heatmaps-pooled-output-1.png)

Figure 5: **Bilaterally pooled RDM** for participant 26: *Walking direction forms the large blocks, map forms the small blocks within. Brackets show number of voxels in each area*
...
<div id="fig-mr-rsa-pooled">

![](/Workbench/scene-areas-hierarchy-rsa/scene-areas-hierarchy-rsa_files/figure-commonmark/fig-mr-rsa-pooled-output-1.png)

Figure 7: **MR-RSA model comparison**: *Mean Cosine Similarity with bilaterally pooled OPA RDM, low-level vs hierarchical models. VIF per predictor shown alongside for each model.*

</div>

<a href="#fig-mr-rsa-pooled" class="quarto-xref">Figure 7</a> shows that the hierarchical model is more similar to OPA RDM than the low-level model, but the 95% confidence intervals still overlap. Thus pooling of bilateral areas leads to the same conclusion reached with the *left V1d* $\to$ *V2d* $\to$ *OPA* pathway.

## Appendix

<details class="code-fold">
<summary>Voxel counts: native atlas mask, post-transform, post-MAD exclusion</summary>
...
Table 1: **Voxel counts per ROI** across processing stages: *atlas mask, post-transform to subject native space, and post-MAD outlier exclusion*
...
![](/Workbench/scene-areas-hierarchy-rsa/scene-areas-hierarchy-rsa_files/figure-commonmark/fig-mds-stress-scree-output-1.png)

Figure 8: **Normalized MDS stress vs. number of dimensions**
...
<div id="ref-Bosch_2025" class="csl-entry">

Bosch, J. J. F. van den, Golan, T., Peters, B., Taylor, J., Shahbazi, M., Lin, B., Charest, I., Diedrichsen, J., Kriegeskorte, N., Mur, M., & Schütt, H. H. (2025). *A Python Toolbox for Representational Similarity Analysis* (p. 2025.05.22.655542). bioRxiv. <https://doi.org/10.1101/2025.05.22.655542>

</div>

<div id="ref-Dale_1999" class="csl-entry">
...
<div id="ref-Epstein_2019" class="csl-entry">
...
<div id="ref-Esteban_2019" class="csl-entry">

Esteban, O., Markiewicz, C. J., Blair, R. W., Moodie, C. A., Isik, A. I., Erramuzpe, A., Kent, J. D., Goncalves, M., DuPre, E., Snyder, M., Oya, H., Ghosh, S. S., Wright, J., Durnez, J., Poldrack, R. A., & Gorgolewski, K. J. (2019). <span class="nocase">fMRIPrep</span>: A robust preprocessing pipeline for functional MRI. *Nature Methods*, *16*(1), 111–116. <https://doi.org/10.1038/s41592-018-0235-4>

</div>

<div id="ref-Fischl_2000" class="csl-entry">
...
<div id="ref-Fischl_2002" class="csl-entry">
...
<div id="ref-Gao_2017" class="csl-entry">

Gao, P., Trautmann, E., Yu, B., Santhanam, G., Ryu, S., Shenoy, K., & Ganguli, S. (2017). *A theory of multineuronal dimensionality, dynamics and measurement* (p. 214262). bioRxiv. <https://doi.org/10.1101/214262>

</div>

<div id="ref-Gorgolewski_2016" class="csl-entry">
...
<div id="ref-Hubel_1962" class="csl-entry">

Hubel, D. H., & Wiesel, T. N. (1962). Receptive fields, binocular interaction and functional architecture in the cat’s visual cortex. *The Journal of Physiology*, *160*(1), 106–154. <https://doi.org/10.1113/jphysiol.1962.sp006837>

</div>

<div id="ref-Li_2016" class="csl-entry">
...
Noda, T., Aschauer, D. F., Chambers, A. R., Seiler, J. P.-H., & Rumpel, S. (2024). Representational maps in the brain: Concepts, approaches, and applications. *Frontiers in Cellular Neuroscience*, *18*. <https://doi.org/10.3389/fncel.2024.1366200>

</div>

<div id="ref-Power_2012" class="csl-entry">
...
<div id="ref-Riesenhuber_1999" class="csl-entry">
...
<div id="ref-Rosenke_2021" class="csl-entry">

<span class="nocase">Rosenke, M., van Hoof, R., van den Hurk, J., Grill-Spector, K., & Goebel, R.</span> (2021). A Probabilistic Functional Atlas of Human Occipito-Temporal Visual Cortex. *Cerebral Cortex*, *31*(1), 603–619. <https://doi.org/10.1093/cercor/bhaa246>

</div>

<div id="ref-Segonne_2004" class="csl-entry">
...
<div id="ref-Serre_2007" class="csl-entry">
...
<div id="ref-Steel_2024" class="csl-entry">

Steel, A. (2024). *Place memory parcels and localizer*.

</div>

<div id="ref-Zhang_2020" class="csl-entry">
...
Zhang, B., & Naya, Y. (2022). A dataset of human <span class="nocase">fMRI</span>/MEG experiments with eye tracking for spatial memory research using virtual reality. *Data in Brief*, *43*, 108380. <https://doi.org/10.1016/j.dib.2022.108380>

</div>

<div id="ref-Zhang_2022a" class="csl-entry">
